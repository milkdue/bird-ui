...
<template>
    <div v-exposure.once="exposureEvent" class="tree-drag-table">
        <ag-grid-vue
            v-if="showContent"
...
export default {
    ...
    methods: {
        ...
        onRowDragMove(event) {
            this.setPotentialParentForNode(event.overNode);
        },
        onRowDragLeave() {
            this.setPotentialParentForNode(null);
        },
        onRowDragEnd(event) {
            if (!this.potentialParent) return;
            let movingData = event.node.data;
            let newParentPath = this.potentialParent.data
        ...
        setPotentialParentForNode(overNode) {
        ...
        cellClassRules() {
            const vm = this;
            return {
                "hover-over": params => params.node === vm.potentialParent
            };
        }
    }
};
